...
class Canvas(
    ...
    init {
        ...
        SwingUtilities.invokeLater { painter.doMondrian(0, 0, width, height) }
    }

    /**
     * Draws a rectangle whose upper-left color is at ([x], [y]) and has width [rectWidth], height
     * [rectHeight], and the specified [outlineColor] and [fillColor].
     */
    fun drawRectangle(
            x: Int,
            y: Int,
    ...
    private class CustomCanvas(private val painter: MondrianPainter, width: Int, height: Int) :
            JPanel() {
        ...
        public val rectangles = mutableListOf<Rectangle>()
        ...
        fun getColorAt(x: Int, y: Int): Color {
            val rgb: Int =
                    bufferedImage?.getRGB(x, y)
                            ?: throw AssertionError("bufferedImage was unexpected null")
            return Color(rgb)
        }
